...
// - SUPPORTS lightmap
Shader "LPCFramework/Environment/WindFlag"
{
Properties {
...
SubShader {
	...
	{	
		...
		//float fObjPhase = dot(transpose( unity_ObjectToWorld)[3].xyz, 1);
	...
	v2f vert (appdata_full v)
	{
		...
		float 		windTime 		= _Time.y * float2(_WindEdgeFlutterFreqScale,1);// wtf
		float4	mdlPos			= AnimateVertex2(v.vertex,v.normal,windParams,wind,windTime);
		

		float time = _Time.y;
		float4 testwave = SmoothTriangleWave((frac( time * float4(1.975, 0.793, 0.375, 0.193) ) * 2.0 - 1.0));
		//float4 newpos = float4(v.vertex.xyz + testwave.xyz,v.vertex.w);
		...
		#ifndef LIGHTMAP_OFF
		o.lmap = v.texcoord1.xy * unity_LightmapST.xy + unity_LightmapST.zw;//what is it 
		#endif
