local _C = UIManager.Controller(UIManager.ControllerName.GiveToPlayerWindow, UIManager.ViewName.GiveToPlayerWindow)
_C.IsPopupBox = true
...
local function ConfirmBtnOnClick()
    if view.NameInput.text == "" then
        UIManager.showTip( { content = Localization.PlayerNameCannotBeEmpty, result = false })
...
function _C:onCreat()
    ...
    Event.addListener(Event.ON_GIVE_TO_PLAYER_SUCCESS, CancelBtnOnClick)
end

function _C:onDestroy()
    Event.removeListener(Event.ON_GIVE_TO_PLAYER_SUCCESS, CancelBtnOnClick)
end