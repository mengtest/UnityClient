...
namespace Rnet
{
    public class RnetStream : Stream
    {
        ...
        private void negotiateKey(bool isReconn)
        {
            ...
            Buffer.BlockCopy(_token, 0, request, 0, 64);
            encryptCipher.XORKeyStream(request, 64, nonce, 0, 8);
            ...
            else
            {
                ...
                for (int n = 72; n > 0;)
                {
                    int x = stream.Read(response, 72 - n, n);
                    if (x == 0)
                        throw new EndOfStreamException();
                    n -= x;
                }
                _ReadCipher.XORKeyStream(response, 0, response, 0, 72);
